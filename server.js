...
app.get('/', (req, res) => {
  res.json({ message: 'Backend is running with Supabase (PostgreSQL & Storage)' });
});
...
app.post('/api/products', upload.array('images', 6), async (req, res) => {
  try {
    ...
    // Upload new images to Supabase (Bucket must exist: 'monika-opticals')
    ...
    if (error) throw error;
    console.log(`  ✅ Product saved to Supabase: "${productData.name}"`);
...
app.put('/api/products/:id', upload.array('images', 6), async (req, res) => {
  try {
    ...
    if (error) throw error;
    console.log(`  ✏️  Product updated in Supabase: "${req.params.id}"`);
    res.json({ ok: true, product: data[0] });
  } catch (err) {
